...
function [parmax, avpar] = mcmc( xdata, ydata, NPAR, lprob_fun, pflg, pset, pmin, pmax, pavg, psig )
...
PARPEN = -1.0;  % minimal penalty for each param
LRG =  1.0e20;
...
par = zeros(NPAR, NPOP);
for m = 1:NPOP
    sigma = (pmax - pmin)/10.0;
    for k = 1:NPAR
        if (pflg(k) >= 0.0)
            par(k,m) = pset(k);
        else
            par(k,m) = pavg(k) + psig(k)*randn(1,1);
            while (par(k,m)<pmin(k) || par(k,m)>pmax(k))
                par(k,m) = pavg(k) + psig(k)*randn(1,1);
            end
        end
    end
...
for i=1:NBETA
    ...
    for m = 1:NPOP
        ptmp = par(:,m);
        lp0 = lprob_fun( xdata, ydata, NPAR, ptmp, pflg, pavg, psig, beta, 1 );
        lprob(m) = lp0;
            ...
            for k = 1:NPAR
                if (pflg(k) >= 0.0) 
                    ...
                    while (nprop < 50)
                        ...
                        if (ptmp(k)+dpar>=pmin(k) && ptmp(k)+dpar<=pmax(k)) 
                            break; 
                        end
                    end
                    if (nprop < 50)
                    ...
                    % new lp
                    lp1 = lprob_fun( xdata, ydata, NPAR, ptmp, pflg, pavg, psig, beta, 1 );
    ...
    for m = 1:NPOP
        ...
        lp1 = lprob_fun( xdata, ydata, NPAR, ptmp, pflg, pavg, psig, beta, 0 );
    ...
    for k = 1:NPAR
        ...
        elseif (parn(k) < 0.2*NANNEAL*NPOP)
            if (sigma(k) > 0.1*sdpar(k))
                sigma(k) = 0.8*sigma(k);
            end
        elseif (parn(k) > 0.8*NANNEAL*NPOP)
            if (sigma(k) < 5.0*sdpar(k))
                sigma(k) = 10.0*sigma(k);
            end
        elseif (parn(k) > 0.55*NANNEAL*NPOP)
            if (sigma(k) < 5.0*sdpar(k))
                sigma(k) = 2.0*sigma(k);
            end
        ...
        if (sigma(k) > (pmax(k) - pmin(k)))
            sigma(k) = pmax(k) - pmin(k);
        end           
    end        
    ...
    if (i == NBETA)
        for k = 1:NPAR
            fprintf('anneal step %d par# %d avg %f std %f prop %f acc %f\n',...
...
% population loop
    ...
    for m = 1:NPOP
        ptmp = par(:,m);
        parmax = par(:,m);
        lp0 = lprob_fun( xdata, ydata, NPAR, ptmp, pflg, pavg, psig, 1.0, 1 );
        lprob(m) = lp0;
        lpmax = lp0;
        parn = initz;    
        for j = 1:NPROP
            ...
            for k = 1:NPAR
                if (pflg(k) >= 0.0) 
                    ...
                    % new lp
                    lp1 = lprob_fun( xdata, ydata, NPAR, ptmp, pflg, pavg, psig, 1.0, 1 );
